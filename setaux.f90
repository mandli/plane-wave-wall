...
!
    ...
    integer :: i_wall
    real(kind=8), parameter :: x_wall = 0.5d0
    real(kind=8), parameter :: wall_height = 1.1d0
    ...
    ! Compute the location of the wall
    topo_finalized = .true.
    if (xlow < x_wall .and. x_wall < xlow + dx * mx) then
        i_wall = floor((x_wall - xlow) / dx + 0.5d0)
    else
        i_wall = -10
    end if
